...
`,le=`# Sovereignty Box Architecture & Value Ladder
...
`,de=`# Dirt: The Distributed Digital Mineral Economy
...
`,ce=`# The Godhead Progression: Thoth, Qabalah, & The 10-Layer Lineage
...
`,ue=`# The W4 Protocol: A 4D Fractal Economic Operating System
...
`,pe=`# YouMeOS Taxonomy & Terminology
...
`,fe=`# Software Manager (Compass Suite)
...
`,Te=`# Magic Mirror: Competitive Benchmarking & Performance Analytics
...
`,Ce=`# Treasure Trove & Treasure Map System
...
| \`includes/class-xophz-compass-magic-formula-wpmudev.php\` | Forminator hook intercepts |
| \`public/class-xophz-compass-magic-formula-public.php\` | Spark registration + manifest |

## COMPASS Admin Sub-App (\`src/routes/magic-formula/\`)

### File Structure

\`\`\`
src/routes/magic-formula/
├── magic-formula.vue              # Root layout (x-sub-app-layout)
├── magic-formula.controller.ts    # Billboard + nav setup
├── magic-formula.routes.ts        # Full route tree with group metadata
├── magic-formula.api.ts           # Typed REST service
├── _magic-formula.scss            # Scoped styles
├── components/
│   ├── listing-layout.vue         # Empty-state template
│   └── formula-placeholder.vue    # Under-construction placeholder
└── views/
    ├── dashboard.vue              # KPI overview + module cards
    ├── forms.vue                  # Form listing → detail drill-down
    ├── form-detail.vue            # Form detail (tabs: overview/submissions/settings)
    ├── polls.vue                  # Poll listing
    ├── quizzes.vue                # Quiz listing
    ├── templates.vue              # Pre-built templates (stub)
    ├── submissions.vue            # Aggregated submissions from all forms
    ├── reports.vue                # Analytics (stub)
    ├── integrations.vue           # CRM/webhook config (stub)
    ├── settings.vue               # Plugin settings (stub)
    └── addons.vue                 # Extensions (stub)
\`\`\`

### Navigation Groups (route meta \`group\`)
...
── config ───────────────
   Integrations       Questbook CRM, webhooks, email
   Settings           Global form defaults, captcha
   Add-ons            Payment gateways, conditional logic
\`\`\`
...
Form submissions are linked to Questbook CRM contacts via \`_qb_raw_email\`. The REST submissions endpoint cross-references each entry's email against \`questbook_contact\` posts and returns the \`_qb_lead_status\` for inline funnel visibility.

## YouMeOS Spark (Future)

The spark (\`apps/youmeos/app/sparks/magic-formula/\`) provides public-facing access to published forms. It uses the **same API** but filters to \`status: publish\` and renders with public-friendly naming (Magic Formulas, Magic Quizzes, Magic Poll-ulas).

The spark will be the first to use the **u-window ↔ x-sub-app-layout bridge** — loading the sub-app layout template inside the u-window content area, with navigation filtered by user role/capabilities.
`,Ae=`# GitHub Plugin Updater
...
`,Me=`# The Hall of the Gods Ecosystem Architecture
...
`,_e=`# Omega Source (Virtual Passport)
...
`,Ee=`# Helios Modules Documentation
...
`,Oe=`# Spark Federation Protocol & Security Architecture
...
`,We=`# YouMeOS: The Asynchronous Cosmos
...
`,Be=`# YouMeOS: The Sovereign Node Architecture
...
`,Ge=`# YouMeOS: Sovereign Identity Architecture 
...
`,He=`# Sub-App Layout & Bazaar Routing Investigation
...
`,qe=`# YouMeOS: The 4D Tesseract Visualization Plan
...
`,Xe=`# Three.js Legacy Migration Guide
...
`,Ke=`# YouMeOS Web3 Vision & Roadmap
...
`,Je=`# Window Management & Workspace Orchestration Roadmap
...
`,Ze=`# YouMeOS Decentralization & Local-First Architecture Roadmap
...
`,en=`# Getting Started with YouMeOS
...
`,nn=`# The Swiss Army Knife for Life
...
`,tn=["innerHTML"],an=H({__name:"x-markdown",props:{content:{}},setup(z){const w=z,p=_(()=>w.content?K.parse(w.content):"");return(T,s)=>(y(),x("div",{class:"x-markdown-container",innerHTML:p.value},null,8,tn))}}),on=Y(an,[["__scopeId","data-v-c9651a1d"]]),rn=80,k=150,V=200,sn=H({__name:"enchiridion-canvas",setup(z){const w=A(null);let p=0,T=null;function s(i,o){const m=Math.random()>.85;return{x:Math.random()*i,y:Math.random()*o,vx:(Math.random()-.5)*.6,vy:(Math.random()-.5)*.6,radius:m?3+Math.random()*3:1+Math.random()*1.5,baseRadius:m?3.5:1.5,hue:m?45:200,pulsePhase:Math.random()*Math.PI*2,pulseSpeed:.02+Math.random()*.03}}return $(()=>{const i=w.value;if(!i)return;const o=i.getContext("2d");if(!o)return;let m=-1e3,u=-1e3,f=0,S=0;const r=[],P=()=>{const e=i.parentElement;if(e){if(i.width=e.clientWidth,i.height=e.clientHeight,f<20&&i.width>50||S<20&&i.height>50)for(const n of r)n.x=Math.random()*i.width,n.y=Math.random()*i.height;f=i.width,S=i.height}};P();for(let e=0;e<rn;e++)r.push(s(i.width,i.height));const E=e=>{const n=i.getBoundingClientRect();m=e.clientX-n.left,u=e.clientY-n.top},D=()=>{m=-1e3,u=-1e3};i.addEventListener("mousemove",E),i.addEventListener("mouseleave",D);const M=()=>{if(!(!o||!i)){o.clearRect(0,0,i.width,i.height);for(const e of r){e.x+=e.vx,e.y+=e.vy,e.pulsePhase+=e.pulseSpeed,e.radius=e.baseRadius+Math.sin(e.pulsePhase)*(e.baseRadius*.4),e.x<0&&(e.x=i.width),e.x>i.width&&(e.x=0),e.y<0&&(e.y=i.height),e.y>i.height&&(e.y=0);const n=e.x-m,t=e.y-u,a=Math.sqrt(n*n+t*t);if(a<V&&a>.1){const v=(V-a)/V;e.vx+=n/a*v*.03,e.vy+=t/a*v*.03}const l=Math.sqrt(e.vx*e.vx+e.vy*e.vy);l>1.2?(e.vx*=.95,e.vy*=.95):l<.2&&(e.vx*=1.05,e.vy*=1.05)}for(let e=0;e<r.length;e++){for(let a=e+1;a<r.length;a++){const l=r[e].x-r[a].x,v=r[e].y-r[a].y;if(Math.abs(l)>k||Math.abs(v)>k)continue;const d=Math.sqrt(l*l+v*v);if(d<k){const g=1-d/k;o.beginPath(),o.moveTo(r[e].x,r[e].y),o.lineTo(r[a].x,r[a].y),o.strokeStyle=`rgba(98, 201, 255, ${g*.25})`,o.lineWidth=1,o.stroke()}}const n=r[e].x-m,t=r[e].y-u;if(Math.abs(n)<k*1.5&&Math.abs(t)<k*1.5){const a=Math.sqrt(n*n+t*t);if(a<k*1.5){const l=1-a/(k*1.5);o.beginPath(),o.moveTo(r[e].x,r[e].y),o.lineTo(m,u),o.strokeStyle=`rgba(217, 190, 111, ${l*.4})`,o.lineWidth=1.5,o.stroke()}}}for(const e of r){if(e.radius<=0)continue;o.beginPath(),o.arc(e.x,e.y,e.radius,0,Math.PI*2),o.fillStyle=`hsla(${e.hue}, 80%, 70%, 0.8)`,o.fill();const n=e.radius*3.5;o.beginPath(),o.arc(e.x,e.y,n,0,Math.PI*2);const t=o.createRadialGradient(e.x,e.y,e.radius,e.x,e.y,n);t.addColorStop(0,`hsla(${e.hue}, 80%, 60%, 0.4)`),t.addColorStop(1,`hsla(${e.hue}, 80%, 60%, 0)`),o.fillStyle=t,o.fill()}p=requestAnimationFrame(M)}},L=new ResizeObserver(P);i.parentElement&&L.observe(i.parentElement),p=requestAnimationFrame(M),T=()=>{cancelAnimationFrame(p),L.disconnect(),i.removeEventListener("mousemove",E),i.removeEventListener("mouseleave",D)}}),Q(()=>{T?.()}),(i,o)=>(y(),x("canvas",{ref_key:"canvasRef",ref:w,class:"enchiridion-canvas"},null,512))}}),ln=Y(sn,[["__scopeId","data-v-37aa8a33"]]),dn={class:"w-100 enchiridion-nav-ui",style:{height:"100%",display:"flex","flex-direction":"column"}},cn={class:"d-flex justify-center align-center opacity-70 mr-4",style:{width:"24px"}},un={class:"px-3 pt-2 pb-2 text-caption text-uppercase font-weight-bold header-label"},hn={class:"h-100 w-100",style:{position:"relative"}},pn={class:"reading-pane mx-auto w-100"},mn="../../../../../docs/",gn=H({__name:"enchiridion-app",setup(z){const w=Object.assign({"../../../../../docs/CHANGELOG.md":te,"../../../../../docs/Cloudflare-Email-Routing.md":ae,"../../../../../docs/Hello-World.md":ie,"../../../../../docs/YouMeOS-Federated-Identity.md":oe,"../../../../../docs/analytics_google-analytics-integration.md":re,"../../../../../docs/authenticity_and_licensing.md":se,"../../../../../docs/business/BOX_TIER_ARCHITECTURE.md":le,"../../../../../docs/business/business_dirt_digital_mineral_economy.md":de,"../../../../../docs/business/business_sephiroth_godhead_progression.md":ce,"../../../../../docs/business/business_w4_protocol_economy.md":ue,"../../../../../docs/business/business_youmeos_decentralized_pwa_model.md":he,"../../../../../docs/compass/blueprints/Core-Taxonomy.md":pe,"../../../../../docs/compass/blueprints/System-Overview.md":me,"../../../../../docs/compass/ecosystem/Patreon-and-Bazaar.md":ge,"../../../../../docs/compass/ecosystem/Portals-and-Ecosystem.md":ye,"../../../../../docs/compass/ecosystem/software_manager.md":fe,"../../../../../docs/compass/plugins/Bomb-Bag-System.md":ve,"../../../../../docs/compass/plugins/Bug-Net-System.md":be,"../../../../../docs/compass/plugins/Lemon-Aid-System.md":we,"../../../../../docs/compass/plugins/Magic-Cloak-System.md":Se,"../../../../../docs/compass/plugins/Magic-Mirror-System.md":Te,"../../../../../docs/compass/plugins/Mirror-Shield-System.md":Pe,"../../../../../docs/compass/plugins/Moving-Castle-System.md":ke,"../../../../../docs/compass/plugins/Treasure-Trove-and-Map-System.md":Ce,"../../../../../docs/compass/plugins/magic-formula-system.md":xe,"../../../../../docs/github-plugin-updater.md":Ae,"../../../../../docs/reference/Ecosystem-Architecture.md":Me,"../../../../../docs/reference/Project-Philosophy.md":Re,"../../../../../docs/reference/versioning.md":Ie,"../../../../../docs/sparks_omegafile.md":_e,"../../../../../docs/youmeos/blueprints/Helios-Modules.md":Ee,"../../../../../docs/youmeos/blueprints/Legacy-Engine-Mechanics.md":De,"../../../../../docs/youmeos/blueprints/OS-Architecture.md":Le,"../../../../../docs/youmeos/blueprints/Spark-Federation-Security.md":Oe,"../../../../../docs/youmeos/blueprints/Unified-Contacts-Blueprint.md":ze,"../../../../../docs/youmeos/core-engine/Asynchronous-Cosmos.md":We,"../../../../../docs/youmeos/core-engine/PWA-Desktop-Node.md":Be,"../../../../../docs/youmeos/core-engine/Sovereign-Identity.md":Ge,"../../../../../docs/youmeos/core-engine/YouMeOS-Engine.md":Ue,"../../../../../docs/youmeos/design-system/Component-Architecture.md":Fe,"../../../../../docs/youmeos/design-system/Component-Refactoring-Guide.md":Ne,"../../../../../docs/youmeos/design-system/Design-System-Atoms.md":Ve,"../../../../../docs/youmeos/design-system/Sub-App-Layout.md":He,"../../../../../docs/youmeos/roadmaps/General-Roadmap.md":Ye,"../../../../../docs/youmeos/roadmaps/Migration-Progress.md":je,"../../../../../docs/youmeos/roadmaps/Tesseract-4D-Visualization.md":qe,"../../../../../docs/youmeos/roadmaps/ThreeJS-Legacy-Migration.md":Xe,"../../../../../docs/youmeos/roadmaps/Web3-Vision-Roadmap.md":Ke,"../../../../../docs/youmeos/roadmaps/WebGPU-Migration.md":$e,"../../../../../docs/youmeos/roadmaps/WebGPU-Shader-Migration.md":Qe,"../../../../../docs/youmeos/roadmaps/Window-Management.md":Je,"../../../../../docs/youmeos/roadmaps/YouMeOS-Decentralization-Roadmap.md":Ze,"../../../../../docs/youmeos/user-guides/youmeos-user-guides-getting-started.md":en,"../../../../../docs/youmeos/user-guides/youmeos_user_guides_swiss_army_knife.md":nn}),p=_(()=>Object.keys(w).map(n=>{const t=n.replace(mn,""),a=t.split("/"),l=a.pop()||"",v=l.replace(".md","").split("-").map(b=>b.charAt(0).toUpperCase()+b.slice(1)).join(" ");let d="03. Platform Docs",g="General";l.toLowerCase()==="hello-world.md"?(d="00. Welcome",g="Start Here"):a.length>=2&&["compass","youmeos"].includes(a[0].toLowerCase())?(d=a[0].toLowerCase()==="youmeos"?"01. YouMeOS":"02. COMPASS",g=a[1]):a.length>0&&(d="03. Platform Docs",g=a[0]);const j=g.replace(/[-_]/g," ").split(" ").map(b=>b.charAt(0).toUpperCase()+b.slice(1).toLowerCase()).join(" ");return{path:n,relativePath:t,title:v,category:j,root:d}}).sort((n,t)=>n.root!==t.root?n.root.localeCompare(t.root):n.category!==t.category?n.category.localeCompare(t.category):n.title.localeCompare(t.title))),T=_(()=>{const n={};p.value.forEach(a=>{n[a.root]||(n[a.root]={}),n[a.root][a.category]||(n[a.root][a.category]=[]),n[a.root][a.category].push(a)});const t={};return Object.keys(n).sort().forEach(a=>{t[a]={},Object.keys(n[a]).sort().forEach(l=>{t[a][l]=n[a][l]})}),t}),s=A(null),i=A(null),o=_(()=>!s.value||!w[s.value]?`# Welcome to the Source

Please select a document from the rail to begin your reading.`:w[s.value]),m=n=>{s.value=n};J(s,()=>{i.value&&(i.value.scrollTop=0)},{flush:"post"});const u=A(null),f=A(null),S=A(0),r=typeof sessionStorage<"u"?sessionStorage.getItem("youmeos:enchiridion:open"):null;let P=null;r&&(P=p.value.find(n=>n.path.toLowerCase().includes(r.toLowerCase())),typeof sessionStorage<"u"&&sessionStorage.removeItem("youmeos:enchiridion:open")),!s.value&&P&&(s.value=P.path,u.value=P.root,f.value=P.category,S.value=1),typeof window<"u"&&window.addEventListener("youmeos:enchiridion:open",n=>{const t=n.detail;if(t){const a=p.value.find(l=>l.path.toLowerCase().includes(t.toLowerCase()));a&&(s.value=a.path,u.value=a.root,f.value=a.category,S.value=1)}});const E=(n,t)=>{u.value=t,f.value=n,S.value=1},D=()=>{S.value=0},M=_(()=>{if(!s.value)return null;const n=p.value.findIndex(t=>t.path===s.value);return n>=0&&n<p.value.length-1?p.value[n+1]:null}),L=()=>{const n=M.value;n&&(m(n.path),(f.value!==n.category||u.value!==n.root)&&(f.value=n.category,u.value=n.root))},e=n=>{const t=n.toLowerCase();return t.includes("start here")?"fal fa-stars":t.includes("blueprints")?"fal fa-drafting-compass":t.includes("user guides")?"fal fa-map-signs":t.includes("core engine")?"fal fa-microchip":t.includes("design system")?"fal fa-swatchbook":t.includes("roadmaps")?"fal fa-road":t.includes("ecosystem")?"fal fa-planet-ringed":t.includes("plugins")?"fal fa-plug":t.includes("reference")?"fal fa-books":t.includes("general")?"fal fa-book-spells":"fal fa-folder"};return(n,t)=>{const a=W("x-spark-splash-action"),l=W("t-spark-splash"),v=W("u-window");return y(),R(v,{id:"enchiridion",title:"Enchiridion",icon:"fal fa-book-spells","default-width":1e3,"default-height":700,"default-nav-pinned":!0},{"nav-content":h(()=>[C("div",dn,[c(ee,{modelValue:S.value,"onUpdate:modelValue":t[1]||(t[1]=d=>S.value=d),transition:"window-fade-transition","reverse-transition":"window-fade-transition",style:{height:"100%",width:"100%"}},{default:h(()=>[c(X,{value:0,style:{height:"100%",width:"100%","overflow-y":"auto","overflow-x":"hidden"},class:"custom-scrollbar"},{default:h(()=>[c(N,{nav:"",density:"compact",class:"pb-8","bg-color":"transparent"},{default:h(()=>[c(I,{title:"The Source",class:B(["mb-1 font-weight-bold category-list-item mt-2",s.value?"opacity-80":"text-primary"]),onClick:t[0]||(t[0]=d=>s.value=null)},{prepend:h(()=>[C("div",cn,[c(ne,{icon:"fal fa-book-spells",class:B(s.value?"":"text-primary")},null,8,["class"])])]),_:1},8,["class"]),(y(!0),x(O,null,G(T.value,(d,g)=>(y(),x(O,{key:g},[C("div",{class:B(["px-3 pt-3 pb-2 text-caption text-uppercase font-weight-bold header-label",g!==Object.keys(T.value)[0]?"mt-4":""])},U(g),3),(y(!0),x(O,null,G(d,(j,b)=>(y(),R(I,{key:b,title:String(b),"prepend-icon":e(String(b)),"append-icon":"fal fa-chevron-right",color:"#62c9ff",class:"mb-1 opacity-80",onClick:yn=>E(String(b),String(g))},null,8,["title","prepend-icon","onClick"]))),128))],64))),128))]),_:1})]),_:1}),c(X,{value:1,style:{height:"100%",width:"100%",display:"flex","flex-direction":"column"}},{default:h(()=>[c(N,{nav:"",density:"compact",class:"flex-grow-1 custom-scrollbar","bg-color":"transparent",style:{"overflow-y":"auto","overflow-x":"hidden"}},{default:h(()=>[c(I,{"prepend-icon":"fal fa-arrow-left",title:"Back",class:"mb-2 back-btn",onClick:D}),C("div",un,[q(U(u.value)+" ",1),t[2]||(t[2]=C("span",{class:"opacity-50 mx-1"},"/",-1)),q(" "+U(f.value),1)]),(y(!0),x(O,null,G(u.value&&f.value&&T.value[u.value]?T.value[u.value][f.value]:[],d=>(y(),R(I,{key:d.path,title:d.title,"prepend-icon":"fal fa-file-alt",active:s.value===d.path,class:"mb-1",color:"#62c9ff",onClick:g=>m(d.path)},null,8,["title","active","onClick"]))),128))]),_:1}),M.value?(y(),R(N,{key:0,nav:"",density:"compact",class:"mt-auto px-2","bg-color":"transparent"},{default:h(()=>[c(I,{"prepend-icon":"fal fa-arrow-right",title:"Next",class:"opacity-80",onClick:L})]),_:1})):Z("",!0)]),_:1})]),_:1},8,["modelValue"])])]),default:h(()=>[C("div",hn,[s.value?(y(),x("div",{key:1,ref_key:"contentScrollEl",ref:i,class:"enchiridion-content fill-height custom-scrollbar"},[C("div",pn,[c(on,{content:o.value},null,8,["content"])])],512)):(y(),R(l,{key:0,icon:"fal fa-book-spells",title:"Enchiridion",subtitle:"The Ultimate Handbook to the","subtitle-highlight":"Youniverse"},{background:h(()=>[c(ln)]),actions:h(()=>[c(F,{cols:"12",sm:"4"},{default:h(()=>[c(a,{title:"Explore",description:"All the knowledge of the Youniverse",icon:"fal fa-compass","icon-color":"#62c9ff"})]),_:1}),c(F,{cols:"12",sm:"4"},{default:h(()=>[c(a,{title:"Learn",description:"Master the tools of the Youniverse",icon:"fal fa-book-open","icon-color":"#D9BE6F"})]),_:1}),c(F,{cols:"12",sm:"4"},{default:h(()=>[c(a,{title:"Dream",description:"Shape the future of your Youniverse",icon:"fal fa-sparkles","icon-color":"#22c55e"})]),_:1})]),_:1}))])]),_:1})}}}),Pn=Y(gn,[["__scopeId","data-v-085807fe"]]);export{Pn as default};
