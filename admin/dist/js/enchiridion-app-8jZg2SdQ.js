...
`,re=`# Authenticity & Licensing System
...
`,le=`# Dirt: The Distributed Digital Mineral Economy
...
3.  **The Reputation Burn:** Transmutation is a destructive alchemical process. Executing the mint permanently burns a specific amount of RP (\`-X RP\`). You are literally spending your built-up community trust to extract financial value.
...
## 4. The Trinity fX Structure (Checks & Balances)

The three governing branches form a triangle — the strongest shape. No single DAO can overrule the other two. They are designed like rock-paper-scissors to create self-stabilizing equilibrium.

### The Three Branches
- **The Labor DAO (Kinetic Forge / Ruby + Emerald holders):** The doers. Builders, hosters, farmers. They govern physical resource allocation, node deployment, and infrastructure.
- **The Wisdom DAO (Sacred Realm / Sapphire + Diamond holders):** The philosophers, educators, and engineers. They govern system architecture, code upgrades, curriculum, and long-term vision.
- **The Arbitration DAO (Sovereign Architecture / Core Crystal holders):** The stabilizing third point. They resolve deadlocks between Labor and Wisdom. They do not micromanage — they serve as the supreme court, ensuring no DAO passes algorithms that violate the core tenets of the Omega Point.

### The Check System
- Labor can override Wisdom if resources are physically unavailable.
- Wisdom can override Labor if execution violates long-term systemic health.
- Arbitration breaks ties — but can itself be dissolved by unanimous agreement of both other DAOs.
- Citizens naturally filter into their governance branch by forging the mineral they are most passionate about.
...
`,he=`# YouMeOS Taxonomy & Terminology
...
`,ge=`# Software Manager (Compass Suite)
...
`,we=`# Magic Mirror: Competitive Benchmarking & Performance Analytics
...
`,Pe=`# Treasure Trove & Treasure Map System
...
`,Ce=`# The Hall of the Gods Ecosystem Architecture
...
`,Me=`# Helios Modules Documentation
...
`,_e=`# Spark Federation Protocol & Security Architecture
...
`,Ee=`# YouMeOS: The Asynchronous Cosmos
...
`,Le=`# YouMeOS: The Sovereign Node Architecture
...
`,Oe=`# YouMeOS: Sovereign Identity Architecture 
...
`,Ue=`# Sub-App Layout & Bazaar Routing Investigation
...
`,Ve=`# YouMeOS: The 4D Tesseract Visualization Plan
...
`,He=`# Three.js Legacy Migration Guide
...
`,je=`# YouMeOS Web3 Vision & Roadmap
...
`,qe=`# WebGPU Shader Migration Plan
...
`,Xe=`# Window Management & Workspace Orchestration Roadmap
...
`,Ke=`# YouMeOS Decentralization & Local-First Architecture Roadmap
...
`,$e=`# Getting Started with YouMeOS
...
`,Je=`# The Swiss Army Knife for Life
...
`,Ze=["innerHTML"],Qe=H({__name:"x-markdown",props:{content:{}},setup(B){const w=B,p=_(()=>w.content?K.parse(w.content):"");return(T,s)=>(y(),x("div",{class:"x-markdown-container",innerHTML:p.value},null,8,Ze))}}),en=j(Qe,[["__scopeId","data-v-c9651a1d"]]),nn=80,k=150,V=200,tn=H({__name:"enchiridion-canvas",setup(B){const w=A(null);let p=0,T=null;function s(i,o){const m=Math.random()>.85;return{x:Math.random()*i,y:Math.random()*o,vx:(Math.random()-.5)*.6,vy:(Math.random()-.5)*.6,radius:m?3+Math.random()*3:1+Math.random()*1.5,baseRadius:m?3.5:1.5,hue:m?45:200,pulsePhase:Math.random()*Math.PI*2,pulseSpeed:.02+Math.random()*.03}}return $(()=>{const i=w.value;if(!i)return;const o=i.getContext("2d");if(!o)return;let m=-1e3,h=-1e3,f=0,S=0;const r=[],P=()=>{const e=i.parentElement;if(e){if(i.width=e.clientWidth,i.height=e.clientHeight,f<20&&i.width>50||S<20&&i.height>50)for(const n of r)n.x=Math.random()*i.width,n.y=Math.random()*i.height;f=i.width,S=i.height}};P();for(let e=0;e<nn;e++)r.push(s(i.width,i.height));const D=e=>{const n=i.getBoundingClientRect();m=e.clientX-n.left,h=e.clientY-n.top},E=()=>{m=-1e3,h=-1e3};i.addEventListener("mousemove",D),i.addEventListener("mouseleave",E);const M=()=>{if(!(!o||!i)){o.clearRect(0,0,i.width,i.height);for(const e of r){e.x+=e.vx,e.y+=e.vy,e.pulsePhase+=e.pulseSpeed,e.radius=e.baseRadius+Math.sin(e.pulsePhase)*(e.baseRadius*.4),e.x<0&&(e.x=i.width),e.x>i.width&&(e.x=0),e.y<0&&(e.y=i.height),e.y>i.height&&(e.y=0);const n=e.x-m,t=e.y-h,a=Math.sqrt(n*n+t*t);if(a<V&&a>.1){const v=(V-a)/V;e.vx+=n/a*v*.03,e.vy+=t/a*v*.03}const l=Math.sqrt(e.vx*e.vx+e.vy*e.vy);l>1.2?(e.vx*=.95,e.vy*=.95):l<.2&&(e.vx*=1.05,e.vy*=1.05)}for(let e=0;e<r.length;e++){for(let a=e+1;a<r.length;a++){const l=r[e].x-r[a].x,v=r[e].y-r[a].y;if(Math.abs(l)>k||Math.abs(v)>k)continue;const d=Math.sqrt(l*l+v*v);if(d<k){const g=1-d/k;o.beginPath(),o.moveTo(r[e].x,r[e].y),o.lineTo(r[a].x,r[a].y),o.strokeStyle=`rgba(98, 201, 255, ${g*.25})`,o.lineWidth=1,o.stroke()}}const n=r[e].x-m,t=r[e].y-h;if(Math.abs(n)<k*1.5&&Math.abs(t)<k*1.5){const a=Math.sqrt(n*n+t*t);if(a<k*1.5){const l=1-a/(k*1.5);o.beginPath(),o.moveTo(r[e].x,r[e].y),o.lineTo(m,h),o.strokeStyle=`rgba(217, 190, 111, ${l*.4})`,o.lineWidth=1.5,o.stroke()}}}for(const e of r){if(e.radius<=0)continue;o.beginPath(),o.arc(e.x,e.y,e.radius,0,Math.PI*2),o.fillStyle=`hsla(${e.hue}, 80%, 70%, 0.8)`,o.fill();const n=e.radius*3.5;o.beginPath(),o.arc(e.x,e.y,n,0,Math.PI*2);const t=o.createRadialGradient(e.x,e.y,e.radius,e.x,e.y,n);t.addColorStop(0,`hsla(${e.hue}, 80%, 60%, 0.4)`),t.addColorStop(1,`hsla(${e.hue}, 80%, 60%, 0)`),o.fillStyle=t,o.fill()}p=requestAnimationFrame(M)}},L=new ResizeObserver(P);i.parentElement&&L.observe(i.parentElement),p=requestAnimationFrame(M),T=()=>{cancelAnimationFrame(p),L.disconnect(),i.removeEventListener("mousemove",D),i.removeEventListener("mouseleave",E)}}),J(()=>{T?.()}),(i,o)=>(y(),x("canvas",{ref_key:"canvasRef",ref:w,class:"enchiridion-canvas"},null,512))}}),an=j(tn,[["__scopeId","data-v-37aa8a33"]]),on={class:"w-100 enchiridion-nav-ui",style:{height:"100%",display:"flex","flex-direction":"column"}},rn={class:"d-flex justify-center align-center opacity-70 mr-4",style:{width:"24px"}},sn={class:"px-3 pt-2 pb-2 text-caption text-uppercase font-weight-bold header-label"},ln={class:"h-100 w-100",style:{position:"relative"}},dn={class:"reading-pane mx-auto w-100"},cn="../../../../../docs/",hn=H({__name:"enchiridion-app",setup(B){const w=Object.assign({"../../../../../docs/CHANGELOG.md":te,"../../../../../docs/Cloudflare-Email-Routing.md":ae,"../../../../../docs/Hello-World.md":ie,"../../../../../docs/YouMeOS-Federated-Identity.md":oe,"../../../../../docs/authenticity_and_licensing.md":re,"../../../../../docs/business/BOX_TIER_ARCHITECTURE.md":se,"../../../../../docs/business/business_dirt_digital_mineral_economy.md":le,"../../../../../docs/business/business_sephiroth_godhead_progression.md":de,"../../../../../docs/business/business_w4_protocol_economy.md":ce,"../../../../../docs/compass/blueprints/Core-Taxonomy.md":he,"../../../../../docs/compass/blueprints/System-Overview.md":ue,"../../../../../docs/compass/ecosystem/Patreon-and-Bazaar.md":pe,"../../../../../docs/compass/ecosystem/Portals-and-Ecosystem.md":me,"../../../../../docs/compass/ecosystem/software_manager.md":ge,"../../../../../docs/compass/plugins/Bomb-Bag-System.md":ye,"../../../../../docs/compass/plugins/Bug-Net-System.md":fe,"../../../../../docs/compass/plugins/Lemon-Aid-System.md":ve,"../../../../../docs/compass/plugins/Magic-Cloak-System.md":be,"../../../../../docs/compass/plugins/Magic-Mirror-System.md":we,"../../../../../docs/compass/plugins/Mirror-Shield-System.md":Se,"../../../../../docs/compass/plugins/Moving-Castle-System.md":Te,"../../../../../docs/compass/plugins/Treasure-Trove-and-Map-System.md":Pe,"../../../../../docs/github-plugin-updater.md":ke,"../../../../../docs/reference/Ecosystem-Architecture.md":Ce,"../../../../../docs/reference/Project-Philosophy.md":xe,"../../../../../docs/reference/versioning.md":Ae,"../../../../../docs/youmeos/blueprints/Helios-Modules.md":Me,"../../../../../docs/youmeos/blueprints/Legacy-Engine-Mechanics.md":Re,"../../../../../docs/youmeos/blueprints/OS-Architecture.md":Ie,"../../../../../docs/youmeos/blueprints/Spark-Federation-Security.md":_e,"../../../../../docs/youmeos/blueprints/Unified-Contacts-Blueprint.md":De,"../../../../../docs/youmeos/core-engine/Asynchronous-Cosmos.md":Ee,"../../../../../docs/youmeos/core-engine/PWA-Desktop-Node.md":Le,"../../../../../docs/youmeos/core-engine/Sovereign-Identity.md":Oe,"../../../../../docs/youmeos/core-engine/YouMeOS-Engine.md":Be,"../../../../../docs/youmeos/design-system/Component-Architecture.md":ze,"../../../../../docs/youmeos/design-system/Component-Refactoring-Guide.md":We,"../../../../../docs/youmeos/design-system/Design-System-Atoms.md":Ge,"../../../../../docs/youmeos/design-system/Sub-App-Layout.md":Ue,"../../../../../docs/youmeos/roadmaps/General-Roadmap.md":Fe,"../../../../../docs/youmeos/roadmaps/Migration-Progress.md":Ne,"../../../../../docs/youmeos/roadmaps/Tesseract-4D-Visualization.md":Ve,"../../../../../docs/youmeos/roadmaps/ThreeJS-Legacy-Migration.md":He,"../../../../../docs/youmeos/roadmaps/Web3-Vision-Roadmap.md":je,"../../../../../docs/youmeos/roadmaps/WebGPU-Migration.md":Ye,"../../../../../docs/youmeos/roadmaps/WebGPU-Shader-Migration.md":qe,"../../../../../docs/youmeos/roadmaps/Window-Management.md":Xe,"../../../../../docs/youmeos/roadmaps/YouMeOS-Decentralization-Roadmap.md":Ke,"../../../../../docs/youmeos/user-guides/youmeos-user-guides-getting-started.md":$e,"../../../../../docs/youmeos/user-guides/youmeos_user_guides_swiss_army_knife.md":Je}),p=_(()=>Object.keys(w).map(n=>{const t=n.replace(cn,""),a=t.split("/"),l=a.pop()||"",v=l.replace(".md","").split("-").map(b=>b.charAt(0).toUpperCase()+b.slice(1)).join(" ");let d="03. Platform Docs",g="General";l.toLowerCase()==="hello-world.md"?(d="00. Welcome",g="Start Here"):a.length>=2&&["compass","youmeos"].includes(a[0].toLowerCase())?(d=a[0].toLowerCase()==="youmeos"?"01. YouMeOS":"02. COMPASS",g=a[1]):a.length>0&&(d="03. Platform Docs",g=a[0]);const Y=g.replace(/[-_]/g," ").split(" ").map(b=>b.charAt(0).toUpperCase()+b.slice(1).toLowerCase()).join(" ");return{path:n,relativePath:t,title:v,category:Y,root:d}}).sort((n,t)=>n.root!==t.root?n.root.localeCompare(t.root):n.category!==t.category?n.category.localeCompare(t.category):n.title.localeCompare(t.title))),T=_(()=>{const n={};p.value.forEach(a=>{n[a.root]||(n[a.root]={}),n[a.root][a.category]||(n[a.root][a.category]=[]),n[a.root][a.category].push(a)});const t={};return Object.keys(n).sort().forEach(a=>{t[a]={},Object.keys(n[a]).sort().forEach(l=>{t[a][l]=n[a][l]})}),t}),s=A(null),i=A(null),o=_(()=>!s.value||!w[s.value]?`# Welcome to the Source

Please select a document from the rail to begin your reading.`:w[s.value]),m=n=>{s.value=n};Z(s,()=>{i.value&&(i.value.scrollTop=0)},{flush:"post"});const h=A(null),f=A(null),S=A(0),r=typeof sessionStorage<"u"?sessionStorage.getItem("youmeos:enchiridion:open"):null;let P=null;r&&(P=p.value.find(n=>n.path.toLowerCase().includes(r.toLowerCase())),typeof sessionStorage<"u"&&sessionStorage.removeItem("youmeos:enchiridion:open")),!s.value&&P&&(s.value=P.path,h.value=P.root,f.value=P.category,S.value=1),typeof window<"u"&&window.addEventListener("youmeos:enchiridion:open",n=>{const t=n.detail;if(t){const a=p.value.find(l=>l.path.toLowerCase().includes(t.toLowerCase()));a&&(s.value=a.path,h.value=a.root,f.value=a.category,S.value=1)}});const D=(n,t)=>{h.value=t,f.value=n,S.value=1},E=()=>{S.value=0},M=_(()=>{if(!s.value)return null;const n=p.value.findIndex(t=>t.path===s.value);return n>=0&&n<p.value.length-1?p.value[n+1]:null}),L=()=>{const n=M.value;n&&(m(n.path),(f.value!==n.category||h.value!==n.root)&&(f.value=n.category,h.value=n.root))},e=n=>{const t=n.toLowerCase();return t.includes("start here")?"fal fa-stars":t.includes("blueprints")?"fal fa-drafting-compass":t.includes("user guides")?"fal fa-map-signs":t.includes("core engine")?"fal fa-microchip":t.includes("design system")?"fal fa-swatchbook":t.includes("roadmaps")?"fal fa-road":t.includes("ecosystem")?"fal fa-planet-ringed":t.includes("plugins")?"fal fa-plug":t.includes("reference")?"fal fa-books":t.includes("general")?"fal fa-book-spells":"fal fa-folder"};return(n,t)=>{const a=z("x-spark-splash-action"),l=z("t-spark-splash"),v=z("u-window");return y(),R(v,{id:"enchiridion",title:"Enchiridion",icon:"fal fa-book-spells","default-width":1e3,"default-height":700,"default-nav-pinned":!0},{"nav-content":u(()=>[C("div",on,[c(ee,{modelValue:S.value,"onUpdate:modelValue":t[1]||(t[1]=d=>S.value=d),transition:"window-fade-transition","reverse-transition":"window-fade-transition",style:{height:"100%",width:"100%"}},{default:u(()=>[c(X,{value:0,style:{height:"100%",width:"100%","overflow-y":"auto","overflow-x":"hidden"},class:"custom-scrollbar"},{default:u(()=>[c(N,{nav:"",density:"compact",class:"pb-8","bg-color":"transparent"},{default:u(()=>[c(I,{title:"The Source",class:W(["mb-1 font-weight-bold category-list-item mt-2",s.value?"opacity-80":"text-primary"]),onClick:t[0]||(t[0]=d=>s.value=null)},{prepend:u(()=>[C("div",rn,[c(ne,{icon:"fal fa-book-spells",class:W(s.value?"":"text-primary")},null,8,["class"])])]),_:1},8,["class"]),(y(!0),x(O,null,G(T.value,(d,g)=>(y(),x(O,{key:g},[C("div",{class:W(["px-3 pt-3 pb-2 text-caption text-uppercase font-weight-bold header-label",g!==Object.keys(T.value)[0]?"mt-4":""])},U(g),3),(y(!0),x(O,null,G(d,(Y,b)=>(y(),R(I,{key:b,title:String(b),"prepend-icon":e(String(b)),"append-icon":"fal fa-chevron-right",color:"#62c9ff",class:"mb-1 opacity-80",onClick:un=>D(String(b),String(g))},null,8,["title","prepend-icon","onClick"]))),128))],64))),128))]),_:1})]),_:1}),c(X,{value:1,style:{height:"100%",width:"100%",display:"flex","flex-direction":"column"}},{default:u(()=>[c(N,{nav:"",density:"compact",class:"flex-grow-1 custom-scrollbar","bg-color":"transparent",style:{"overflow-y":"auto","overflow-x":"hidden"}},{default:u(()=>[c(I,{"prepend-icon":"fal fa-arrow-left",title:"Back",class:"mb-2 back-btn",onClick:E}),C("div",sn,[q(U(h.value)+" ",1),t[2]||(t[2]=C("span",{class:"opacity-50 mx-1"},"/",-1)),q(" "+U(f.value),1)]),(y(!0),x(O,null,G(h.value&&f.value&&T.value[h.value]?T.value[h.value][f.value]:[],d=>(y(),R(I,{key:d.path,title:d.title,"prepend-icon":"fal fa-file-alt",active:s.value===d.path,class:"mb-1",color:"#62c9ff",onClick:g=>m(d.path)},null,8,["title","active","onClick"]))),128))]),_:1}),M.value?(y(),R(N,{key:0,nav:"",density:"compact",class:"mt-auto px-2","bg-color":"transparent"},{default:u(()=>[c(I,{"prepend-icon":"fal fa-arrow-right",title:"Next",class:"opacity-80",onClick:L})]),_:1})):Q("",!0)]),_:1})]),_:1},8,["modelValue"])])]),default:u(()=>[C("div",ln,[s.value?(y(),x("div",{key:1,ref_key:"contentScrollEl",ref:i,class:"enchiridion-content fill-height custom-scrollbar"},[C("div",dn,[c(en,{content:o.value},null,8,["content"])])],512)):(y(),R(l,{key:0,icon:"fal fa-book-spells",title:"Enchiridion",subtitle:"The Ultimate Handbook to the","subtitle-highlight":"Youniverse"},{background:u(()=>[c(an)]),actions:u(()=>[c(F,{cols:"12",sm:"4"},{default:u(()=>[c(a,{title:"Explore",description:"All the knowledge of the Youniverse",icon:"fal fa-compass","icon-color":"#62c9ff"})]),_:1}),c(F,{cols:"12",sm:"4"},{default:u(()=>[c(a,{title:"Learn",description:"Master the tools of the Youniverse",icon:"fal fa-book-open","icon-color":"#D9BE6F"})]),_:1}),c(F,{cols:"12",sm:"4"},{default:u(()=>[c(a,{title:"Dream",description:"Shape the future of your Youniverse",icon:"fal fa-sparkles","icon-color":"#22c55e"})]),_:1})]),_:1}))])]),_:1})}}}),vn=j(hn,[["__scopeId","data-v-085807fe"]]);export{vn as default};
